...
public class ${name}Feature extends ${generator.map(featuretype, "features")} {
			...
			public static final RegistryObject<Feature<?>> ${data.getModElement().getRegistryNameUpper()} =
				register("${data.getModElement().getRegistryName()}", ${data.getModElement().getName()}Feature::data,
						new FeatureRegistration(GenerationStep.Decoration.${generator.map(data.generationStep, "generationsteps")},
							${data.getModElement().getName()}Feature.GENERATE_BIOMES,
							${data.getModElement().getName()}Feature::placedFeature)
				);
